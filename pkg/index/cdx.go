...
func NewCdxRecord(wr warcrecord.WarcRecord, fileName string, offset int64, rle int) *cdx.Cdx {
	cdx := &cdx.Cdx{
	...
	case *warcrecord.RevisitBlock:
		if resp, err := v.Response(); err == nil {
			cdx.Hsc = strconv.Itoa(resp.StatusCode)
			cdx.Mct = resp.Header.Get(warcrecord.ContentType)
			cdx.Ple = resp.Header.Get(warcrecord.ContentLength)
		}
	case warcrecord.ResourceBlock:
		cdx.Mct = wr.WarcHeader().Get(warcrecord.ContentType)
	}
